...
var MapManager = (function($, d3, leaflet) {
  return (
    function(eventData, campaignOffices, zipcodes, options) {
      ...
      var mapboxTiles = leaflet.tileLayer('https://maps.wikimedia.org/osm-intl/{z}/{x}/{y}.png' , { attribution: '<a href="http://www.openstreetmap.org/copyright" target="_blank">&copy; OpenStreetMap contributors</a>'});
 ...
 var GREEN_ICON = L.icon({
  ...
  var BLUE_ICON = L.icon({
	...
	//shadowUrl: '//map.aufstehen.io/img/leaf-shadow.png',
      ...
      var defaultCoord = options&&options.defaultCoord ? options.defaultCoord : {center: [51.8, 10.25], zoom: 7};
    ...
    */
    ...
    module.filter = function(zipcode, distance, sort, filterTypes) {
      ...
      if (targetZipcode == undefined || !targetZipcode) {
        $("#event-list").append("<li class='error lato'>Zipcode does not exist. <a href=\"https://jetzt.aufstehen.de/page/event/search_results?orderby=zip_radius&zip_radius%5b0%5d=" + zipcode + "&zip_radius%5b1%5d=100&country=DE&radius_unit=mi\">Try our events page</a></li>");
        return;
      }
...
(function($) {
  ...
  $(document).on("submit", "form.event-form", function() {
    ...
    $.ajax({
      cache: true,
      type: 'POST',
      url: 'https://jetzt.aufstehen.de/page/graph/addrsvp',
